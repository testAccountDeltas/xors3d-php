...
Type tRowDominoFirst
	Field entity%
End Type

Global lastDomino.tSpiralDomino
Global lastDominoIndex

xCreateLog(LOG_HTML, LOG_INFO, "Domino2.html")

xGraphics3D 1024, 768, 32, False, 0;True
xHidePointer()
...
;xSetEngineSetting("Shadows::Blur", "true")

GenerateRowDominos()
GenerateSpiralDominos()
Reset()
...
While Not xKeyDown(key_escape)
	...
	xText( 10, 30, "Dominos: " + maxDominos + "+" + (maxDominosX * maxDominosZ))
	If launched
...
Function Launch()
	If launched
		Return
	EndIf
	For rd.tRowDominoFirst = Each tRowDominoFirst
		xEntityApplyImpulse(rd\entity, 0, 0, 2.0, 0, domSize * 1.2, 0, False, False)
	Next
	sd.tSpiralDomino = First tSpiralDomino
	xEntityApplyImpulse(sd\entity, 0, 0, 5.0, 0, domSize, 0, False, False)
	launched = True
End Function
...
Function Reset()
	...
	rd.tRowDomino = First tRowDomino
	For x =  0 To maxDominosX  - 1
		For z =  0 To maxDominosZ  - 1
			lX = (x - maxDominosX / 2) * domStepX
			lZ = (z - maxDominosX / 2) * domStepZ
			xEntityReleaseForces(rd\entity)
			xEntitySleep(rd\entity)
			xPositionEntity(rd\entity, lX, domSize, lZ)
			xRotateEntity(rd\entity, 0, 0, 0)
			rd = After rd
		Next
	Next
	
	For sd.tSpiralDomino = Each tSpiralDomino
		lX = lRadius * Cos(lAngle)
		lZ = lRadius * Sin(lAngle)
		xEntityReleaseForces(sd\entity)
		xPositionEntity(sd\entity, lX, domSize, lZ)
		xRotateEntity(sd\entity, 0.0, lAngle, 0.0)
		xEntityColor(sd\entity, 255, 255, 255)
		lAngle = lAngle + domDeltaAngle / lRadius * domStartRadius / domDeltaRadius
		lRadius = lRadius + domDeltaRadius
	Next
	lastDomino = First tSpiralDomino
	lastDominoIndex = 0
	rCol = 255
	gCol = 255
	bCol = 255
...
Function GenerateRowDominos()
	Local lMasterReady = False
	For x = 0 To maxDominosX  - 1
		For z = 0 To maxDominosZ  - 1
			d.tRowDomino = New tRowDomino
			If Not lMasterReady
				...
				lMasterReady = True
			Else
				Local master.tRowDomino = First tRowDomino
				d\entity = xCreateInstance(master\entity)
			EndIf
			If z = 0
				fir.tRowDominoFirst = New tRowDominoFirst
				fir\entity = d\entity
			EndIf
			xScaleEntity(d\entity, domSize * 1.0, domSize * 1.2, domSize * 0.1)
			xEntityAddBoxShape(d\entity, 1)
			If ((x+z) Mod 2)
				xEntityColor(d\entity, 255, 128, 32)
			Else
				xEntityColor(d\entity, 32, 32, 32)
			EndIf
		Next
	Next
End Function

Function GenerateSpiralDominos()
	For i =  0 To maxDominos  - 1
		d.tSpiralDomino = New tSpiralDomino
		If i = 0
			d\entity = xCreateCube()
			xSetEntityEffect d\entity, inst_shader
			xSetEffectTechnique d\entity, "Instancing"
		Else
			Local master.tSpiralDomino = First tSpiralDomino
			d\entity = xCreateInstance(master\entity)
		EndIf
...
Function UpdateSpiralDominos()
	If Not launched
		Return
	EndIf
	Local delta = 1024 / maxDominos
	Local d.tSpiralDomino = lastDomino
	While d <> Null

;*******************************************************************
;*                                                                 *
;* Xors3D Engine. Glass sample, (c) 2009 Xors3D Team               *
;* www: http://xors3d.com                                          *
...
While Not xKeyHit(1) Or xWinMessage("WM_CLOSE")
	...
	; camera controll
	If xKeyDown(KEY_UP)    Then xTurnEntity Piv,  1.0,  0.0, 0.0, True
	If xKeyDown(KEY_DOWN)  Then xTurnEntity Piv, -1.0,  0.0, 0.0, True
	If xKeyDown(KEY_LEFT)  Then xTurnEntity Piv,  0.0,  1.0, 0.0, True
	If xKeyDown(KEY_RIGHT) Then xTurnEntity Piv,  0.0, -1.0, 0.0, True

	; glass color controll
	cl# = 0.01
	If xKeyDown(KEY_Q) Then r = r + cl
	If xKeyDown(KEY_A) Then r = r - cl
	If xKeyDown(KEY_W) Then g = g + cl
	If xKeyDown(KEY_S) Then g = g - cl
	If xKeyDown(KEY_E) Then b = b + cl
	If xKeyDown(KEY_D) Then b = b - cl
	If (r > 1.0) Then r = 1.0
	If (r < 0.0) Then r = 0.0
	If (g > 1.0) Then g = 1.0
	If (g < 0.0) Then g = 0.0
	...
	; falloff controll
	cl# = 0.03
	If xKeyDown(KEY_R) Then FallOffPow = FallOffPow + cl
	If xKeyDown(KEY_F) Then FallOffPow = FallOffPow - cl

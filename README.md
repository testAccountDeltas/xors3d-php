...
- **Настройки** (сохраняются в `xors3d-php/craft-settings.json`):
  чувствительность мыши, инверсия Y, FOV, туман, день/ночь, **bloom** (шейдер),
  громкость звука, дальность прорисовки (вживую); разрешение и vsync (при перезапуске); размер

...
- **Генеративный стриминг**: мир хранится как данные (карта высот + руды
  детерминированным шумом + оверлей правок), а блоки-сущности создаются только
  в чанках вокруг игрока и освобождаются при отдалении. Под землёй/вдали
  ничего не рисуется зря; туман маскирует границу. Мир до 128×128 стартует
  быстро и держит FPS независимо от размера — регулируется ползунком
  «Render distance» (меньше = стабильные 60).

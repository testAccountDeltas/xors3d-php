...
- **Меширование чанков**: каждый чанк — один объединённый меш только из видимых
  граней (скрытые между блоками грани не строятся), с отсечением задних граней.
  Это на порядок меньше draw calls, чем отдельные кубы, — стабильные ~60 FPS.
  (Узкое место было именно в числе сущностей, а не в FFI: замер показал ~3 млн
  FFI-вызовов/сек, бюджет ~50k вызовов на кадр.)
